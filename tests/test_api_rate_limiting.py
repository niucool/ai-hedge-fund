...
class TestRateLimiting:
    ...
    def test_handles_single_rate_limit(self, mock_get, mock_sleep):
        ...
        mock_get.assert_has_calls([call(url, headers=headers, params=None), call(url, headers=headers, params=None)])
    ...
    def test_handles_post_rate_limiting(self, mock_post, mock_sleep):
        ...
        mock_post.assert_has_calls([call(url, headers=headers, json=json_data, params=None), call(url, headers=headers, json=json_data, params=None)])
    ...
    def test_ignores_other_errors(self, mock_get, mock_sleep):
        ...
        # Call the function
        headers = {"X-API-KEY": "test-key"}
        url = "https://api.newsdata.io/test"
    ...
    def test_full_integration(self, mock_get, mock_sleep, mock_cache):
        """Test that get_company_news function properly handles rate limiting."""
        ...
        mock_200_response = Mock()
        mock_200_response.status_code = 200
        mock_200_response.json.return_value = {"results": [{"title": "Apple news", "creator": ["John"], "source_id": "test", "pubDate": "2024-01-01 10:00:00", "link": "https://test.com"}]}

        mock_get.side_effect = [mock_429_response, mock_200_response]

        # Set environment variable for API key
        with patch.dict(os.environ, {"NEWSDATA_API_KEY": "test-key"}):
            # Call get_company_news
            from src.tools.api import get_company_news

            result = get_company_news("AAPL", "2024-01-02", start_date="2024-01-01")
        ...
        mock_cache.get_company_news.assert_called_once()
